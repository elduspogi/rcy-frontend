...
const RegisterDonorContent = () => {
  ...
  const onSubmit = (payload) => {
    axiosClient.post('/register/donor', payload)
      .then(({data}) => {
        console.log(data);
      })
      .catch((err) => {
        ...
        <div className="pb-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <ContactNumber length={3} placeholder='Enter Age' register={register} name='age'  />
            </div>

            <div>
              <InputLabel id='birthdate' text='Birthdate' isImportant={true} />
              <Birthdate col='2' register={register} />
            </div>

            <div>
              <InputLabel id='selectBloodType' text='Blood Type' isImportant={true} />
              <Select defaultValue="Select Blood Type" col="2" options={types} register={register} name='blood_type' />
            </div>

            <div>
              <InputLabel id='weight' text='Weight (kg)' isImportant={true} />
              <ContactNumber length={3} placeholder='Enter weight' register={register} name='weight' />
            </div>
          </div>
        </div>
